<template>
  <div class="home">
    ...
    <div class="container">
      <el-card class="box-card">
        ...
        >
          ...
          <el-table-column prop="name" align="center" label="部门名称" width="350">
            <template v-slot="scope">
          ...
          <el-table-column prop="operation" align="center" label="操作" fixed="right">
            <template v-slot="scope">
              <el-button icon="edit" size="small" type="text" @click="onEditDepartment(scope.row)"
                >修改</el-button
              >
              <el-button
                icon="delete"
    ...
    <!-- 弹框页面 -->
    <DialogDepartment :dialog="dialog" :form="form" @update="getProfile"></DialogDepartment>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getProfile () {
      // 获取数据
      this.$axios('/api/department').then(res => {
    ...
    onDeleteDepartment (row, index) {
      // 删除
      if (!this.role) {
        return this.$message.error('没有权限!')
      }
      this.$axios.delete(`/api/department/delete/${row._id}`).then(res => {
        this.$message('删除成功')
        this.getProfile()
      })
    },
